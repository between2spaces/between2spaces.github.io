...
class Game {
	constructor(container) {
		...
		window.addEventListener('keydown', (event) => {
			if (moveMapKeys.hasOwnProperty(event.key)) {
				this.map.centreNeighbour(moveMapKeys[event.key]);
			}
		});
	...
	run() {
		const self = this;
		this.intervalId = window.setInterval(() => self.update(), 1000);
	}
...
class Map {
	constructor(container) {
		...
		this.position();
		const self = this;
		window.addEventListener('resize', () => self.position(), true);
	}
