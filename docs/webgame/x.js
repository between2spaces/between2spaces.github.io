...
export default class Client {
	...
	constructor( client ) {
		...
		this.socket = null; this.client = client; this.client.interval
			??= 10000; this.callbacks = {}; this.cached = '';
	...
	onOpen() {
		...
		client.update && ( this.uiid ??= setInterval( () =>
			client.update( client ), client.interval ) );
	...
	onMessage( message ) {
		...
		const [ callerId, cid, fn, ...args ] =
			message.toString().split( '_' );

		if ( fn in this.callbacks ) {

			console.log( `callbacks[ '${fn}' ].resolve` );
			this.callbacks[ fn ].resolve( args ); delete
				this.callbacks[ fn ]; return;
		...
		this.client.debug && console.error( `Warn:
	...
	call( target, fn, args = '' ) {
		...
		args = args?.constructor === Array ? args.join( '_' ) :
			`${args}`;

		this.callbacks[ cid ] = {};

		const msg = ( target ?? '' ) + '_' + cid + '_' + fn + ( args ?
			`_${args}` : '' );
		...
		return new Promise( function ( resolve, reject ) {
			...
			this.socket.readyState ? this.socket.send( msg ) : (
				this.cached += ! this.cached ? msg : ';' +
				msg);
	...
	propertiesOf( name ) {
		...
		return new Promise( ( resolve, reject ) => name in
			this.cachedProperties ? resolve() : reject()).then( ()
				=> this.cachedProperties[ name ], () =>
				this.call( 'Entity', 'properties',
					'Tree').then( ( properties ) => {
...
function swpAddProperty( swp, property, client ) {
	...
	const arr = client[ property ]; arr?.length && swp.push( property +
		arr.join( '_' ).replace( ' ', '' ) );
